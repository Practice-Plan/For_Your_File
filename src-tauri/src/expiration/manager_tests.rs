...
mod tests {
    use crate::expiration::{
        format_expiration_date, format_remaining_time, ExpirationConfig, ExpirationManager,
        ExpirationStatus,
    };
    ...
    fn test_check_expired_entries_empty() {
        ...
        let expired = manager
            ...
            .expect("Failed to check expired");
    ...
    fn test_get_expiring_soon() {
        ...
        let expiring = manager
            .get_expiring_soon()
            .expect("Failed to get expiring soon");
    ...
    fn test_count_expired() {
        ...
        for i in 0..3 {
            let mut entry =
                create_test_entry(&format!("C:/test_expired_{}.lnk", i), "C:/target.exe");
            entry.expires_at = Some((Utc::now() - Duration::days(1)).timestamp());
            insert_entry(&conn, &entry);
        }
        ...
        for i in 0..2 {
            let mut entry =
                create_test_entry(&format!("C:/test_active_{}.lnk", i), "C:/target.exe");
            entry.expires_at = Some((Utc::now() + Duration::days(7)).timestamp());
            insert_entry(&conn, &entry);
        }
    ...
    fn test_delete_all_expired() {
        ...
        for i in 0..2 {
            let mut entry =
                create_test_entry(&format!("C:/test_del_expired_{}.lnk", i), "C:/target.exe");
            entry.expires_at = Some((Utc::now() - Duration::days(1)).timestamp());
            insert_entry(&conn, &entry);
        }
